...
def get_amazon_url_from_google_search(search_url):
    ...
    for link in soup.find_all('a'):
        ...
        # Found an amazon url
        if ('www.amazon.com' in url or 'www.amazon.in' in url) and ('/dp/' in url):
...
def query_google_books_api(book_data, type = "isbn", debug = False):
    '''
    ...
    if(type == "isbn"):
        rest_url = 'https://www.googleapis.com/books/v1/volumes?key='+google_books_api_key+'&q=isbn:' + book_data
    else:
        rest_url = 'https://www.googleapis.com/books/v1/volumes?key='+google_books_api_key+'&q=' + book_data
