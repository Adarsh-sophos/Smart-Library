...
sys.path.append(main.BASE_PATH + '/models')

import book_functions, scraper, server, process


def format_file_path_for_routing(file_path):
    file_path = file_path.replace(main.BASE_PATH, '')
    return file_path
...
def index():
    ...
    if(method == 'POST'):
        ...
        if(file):
            ...
            books = process.full_pipeline(raw_file_path, submission_id)
